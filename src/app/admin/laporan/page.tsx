      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6 relative z-10">
          <div className="bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 px-6 py-3 rounded-2xl flex items-center gap-3">
            <span className="text-sm text-slate-600 dark:text-slate-400 font-medium tracking-wide">Total Lulusan:</span>
            <span className="text-xl font-black text-slate-800 dark:text-white">{data?.totalLulusan || 0}</span>
          </div>

          <div className="flex gap-3 w-full sm:w-auto">
            <a
              href={`/api/admin/export${selectedAngkatan ? `?angkatanId=${selectedAngkatan}` : ""}`}
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-600 text-white px-6 py-3 rounded-xl text-sm font-bold shadow-lg hover:shadow-xl dark:shadow-[0_0_20px_rgba(16,185,129,0.3)] transition-all duration-300 transform hover:-translate-y-0.5"
            >
              <Download className="h-4 w-4" /> Export CSV
            </a>
            <Link
              href={`/admin/cetak-surat${selectedAngkatan ? `?angkatanId=${selectedAngkatan}` : ""}`}
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-500 to-blue-600 text-white px-6 py-3 rounded-xl text-sm font-bold shadow-lg hover:shadow-xl dark:shadow-[0_0_20px_rgba(99,102,241,0.3)] transition-all duration-300 transform hover:-translate-y-0.5"
            >
              <Printer className="h-4 w-4" /> Cetak Surat LL
            </Link>
          </div>
        </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50/50 dark:bg-white/5 backdrop-blur-md text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">
                <tr>
                  ...
                  <th className="px-6 py-4">Konsentrasi</th>
                  <th className="px-6 py-4 text-center">Toga</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200 dark:divide-white/5">
                {data?.mahasiswaList.map((m: any, idx: number) => (
                  <tr key={m._id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.03] transition-colors duration-200">
                    ...
                    <td className="px-6 py-4 text-xs font-mono text-slate-500">{m.nik || "-"}</td>
                    <td className="px-6 py-4 text-xs text-slate-600 dark:text-slate-300">
                      {m.tempatLahir}, {m.tanggalLahir ? new Date(m.tanggalLahir).toLocaleDateString("id-ID") : "-"}
                    </td>
                    <td className="px-6 py-4 text-xs font-medium text-slate-700 dark:text-slate-300">{m.konsentrasi || "-"}</td>
                    <td className="px-6 py-4 text-center">
                      <span className="inline-flex items-center justify-center bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1 font-bold text-xs">
                        {m.ukuranToga || "-"}
                      </span>
                    </td>
                  </tr>
                ...
                {data?.mahasiswaList.length === 0 && (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                      Belum ada data wisudawan terverifikasi.
                    </td>
                  </tr>
                )}
              </tbody>
